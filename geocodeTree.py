...
class GeocodeTree:
    ...
    def save_and_return(self):
        for i in range(len(self.tree_list)-1):
            self.G.add_edge(self.tree_list[i], self.tree_list[i+1])
            self.G.add_edge(self.tree_list[i+1], self.tree_list[i])

        self.G.add_edge(self.tree_list[0], self.start_node)
        self.G.add_edge(self.start_node, self.tree_list[0])
        self.G.add_edge(self.tree_list[-1], self.end_node)
        self.G.add_edge(self.end_node, self.tree_list[-1])
